...
def test_fit_all_models():
    ...
    config = yaml.safe_load(
        open(
            "/home/matteo/projects/cfdna/wfs/src/tmp-cfclone/cfclone/tests/test_fit_output_laplace/test_fit_output_laplace.yaml",
            "r",
        )
    )
    ...
    if len(chroms) > 0 and "all" not in chroms:
    ...
    if len(clones) > 0 and "all" not in clones:
    ...
    for use_outlier in [True, False]:
        for use_rdr in [True, False]:
            for use_baf in [True, False]:
                ...
                exec_dir_model = (
                    str(out_dir_model.joinpath("exec_dir")) if exec_dir else None
                )

                laplace_dir_model = (
                    str(out_dir_model.joinpath("laplace_exec_dir"))
                    if laplace_exec_dir
                    else None
                )
